  ...
  async (credentials, { rejectWithValue }) => {
    try {
      ...
      if (!response.ok) {
        ...
        return rejectWithValue(errorData.error?.message || "Error inesparado en la autenticación. Intenta de nuevo.");
      }
      ...
      const errMsg =
        error instanceof Error
          ? error.message
          : "Error inesparado en la autenticación. Intenta de nuevo.";
...
export const registerUser = createAsyncThunk<IUserAuthResponse, IUserRegisterRequest>(
  ...
  async (newUser, { rejectWithValue }) => {
    try {
      ...
      if (!response.ok) {
        const errorData: IUserAuthError = await response.json();
        return rejectWithValue(errorData.error?.message || "Error inesparado en el registro. Intenta de nuevo.");
      }
      ...
      const errMsg =
        error instanceof Error
          ? error.message
          : "Error inesparado en el registro. Intenta de nuevo.";
      return rejectWithValue(errMsg);
    }
  }
);
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(loginUser.fulfilled, (state, action: PayloadAction<IUserAuthResponse>) => {
        ...
        localStorage.setItem("userRole", action.payload.data.response.role.toString());
