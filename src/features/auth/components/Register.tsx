...
export default function RegisterPage() {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>): Promise<boolean> => {
    ...
    // Asignar campos requeridos por step
    const requiredFieldsByStep: Record<number, (keyof IUserRegisterRequest)[]> = {
      0: ["email", "password"],
      1: ["name", "lastName"],
    ...
    // Buscar el primer step con un campo faltante
    let firstMissingStep: number | null = null;

    for (const [step, fields] of Object.entries(requiredFieldsByStep)) {
      const missingInThisStep = fields.some((field) => !form[field]);
      if (missingInThisStep) {
        firstMissingStep = Number(step);
        break;
      }
    }
  ...
  return (
    <Container>
      <FormWrapper>
        ...
        {error && <ErrorText>¡Ups! {error}</ErrorText>}
      </FormWrapper>
    </Container>
  );
}
